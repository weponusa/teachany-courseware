...
**3.1 L1 互动课件（必选）**：
- **必须使用 Section 10.2.1 的 HTML 骨架模板**，禁止自行发明页面结构。呈现形态为**单一连续网页**（模块自上而下连续排列、整页滚动），导航使用 Sticky 顶部导航，**不得**使用前后翻页或分页放映（见 10.2.2）
...
□ 17. 知识图谱溯源：课件核心内容是否可追溯到知识图谱/Web搜索/明确标注的来源？
     → ✅/❌ ___
□ 18. 版式一致性：是否使用了 Section 10.2.1 标准 HTML 骨架模板？是否为**单一连续网页**（无整屏分页 / 无滚动吸附 / 无前后翻页条）？主容器宽度是否统一为 1080px 并水平居中对齐？
     → ✅/❌ ___
