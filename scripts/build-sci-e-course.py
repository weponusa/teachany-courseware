#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate elementary science courseware from a JSON spec.

Usage:
  python3 build-sci-e-course.py --spec path/to/spec.json
...
<title>{esc(spec["title"])} · 小学科学 G{grade} · TeachAny v7.20</title>
...
<meta name="course-grade" content="elementary-{grade}">
...
<meta name="teachany-lesson-type" content="concept-inquiry">
...
def build_manifest(spec: dict) -> dict:
    ...
    return {
        ...
        "subject": "science", "grade": str(spec["grade"]), "stage": "elementary",
        "domain": spec.get("domain", "life-science"),
        "lesson_type": "concept-inquiry", "status": "community", "author": "TeachAny",
...
def build_plan(spec: dict) -> str:
    ...
    return f"""# {nid} · {spec["title"]}（小学科学 G{spec["grade"]}）
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build elementary science courseware from JSON spec")
