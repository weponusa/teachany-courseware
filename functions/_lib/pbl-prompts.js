...
## 可执行任务步骤（硬性）
- 每个 phase 的 steps 至少 2 条，每条须同时包含：**动词 + 操作对象 + 工具/数据/方法 + 可检查产出**
- 禁止空话：「进行调研」「完成探究」「培养素养」「开展活动」「运用知识点完成任务」
- 若题目是口号/品牌/抽象表述（如「环球卓越教育」），必须**落地为 1 个具体交付物**（校园调查展板、班级活动策划书、对比报告等），steps 写清谁做什么、用什么表/工具、交什么稿
...
- 推荐方案 phases 4-5 个
- 每个 steps 条目 ≥15 字，须含可操作动词（设计/测量/撰写/访谈/统计/绘制…）并与阶段 deliverable 挂钩
