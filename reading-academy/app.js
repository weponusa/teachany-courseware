...
function renderLevel(){renderSteps();scoreBadge.textContent='积分 '+state.score;state.answered=false;const lv=DATA.levels[state.idx];const hasImg=lv.img;const imgHtml=hasImg?'<div class="stage"><div class="loading">正在生成场景插画…</div><img alt="场景插画" src="https://image.pollinations.ai/prompt/'+encodeURIComponent(lv.img+'，children picture book style, warm colors, no text')+'?width=800&height=500&nologo=true" onerror="this.style.display=\\'none\\';if(this.previousElementSibling)this.previousElementSibling.textContent=\\'插画生成中…\\'" onload="if(this.previousElementSibling)this.previousElementSibling.remove()"></div>':'';scene.innerHTML='<span class="scene-tag">'+lv.tag+'</span>'+imgHtml+'<p class="story">'+lv.story+'</p><div class="qbox"><p class="q">'+lv.q+'</p><p class="hint">'+lv.hint+'</p><div class="opts" id="opts"></div><div class="feedback" id="feedback"></div></div><div class="nav"><button class="btn btn-ghost" id="prevBtn" '+(state.idx===0?'style="display:none"':'')+'>&larr; 上一关</button><button class="btn btn-primary" id="nextBtn" disabled>'+(state.idx===DATA.levels.length-1?'查看结果 ✦':'下一关 &rarr;')+'</button></div>';const opts=$('#opts');shuffle(lv.opts.slice()).forEach(o=>{const b=document.createElement('button');b.className='opt';b.textContent=o.t;b.onclick=()=>answer(b,o,lv);opts.appendChild(b)});$('#nextBtn').onclick=()=>{state.idx===DATA.levels.length-1?showEnd():(state.idx++,renderLevel())};const prev=$('#prevBtn');if(prev)prev.onclick=()=>{state.idx--;renderLevel()}}
